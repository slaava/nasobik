...
const GAMES: { mode: GameMode; glyph: ReactNode; label: string; enabled: (p: Profile) => boolean }[] = [
  { mode: 'tables', glyph: '× ÷', label: 'Násobení', enabled: () => true },
  { mode: 'arith', glyph: '+ −', label: 'Sčítání a odčítání', enabled: p => p.arithEnabled },
  { mode: 'clock', glyph: <ClockIcon className="inline-block w-6 h-6" />, label: 'Hodiny', enabled: p => p.clockEnabled },
]
...
export function HomeScreen({ profile, cards, sessions, onPlay, onParent }: Props) {
  ...
  return (
    <div className="relative flex flex-col h-full bg-paper text-ink px-4 pt-10 pb-4 lg:max-w-xl lg:mx-auto">
      <button type="button" onClick={onParent} aria-label="Pro rodiče" className="absolute top-3 right-4 text-ink"><GearIcon className="w-7 h-7" /></button>
