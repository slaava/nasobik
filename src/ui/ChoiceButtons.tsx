      ...
      {options.map(value => (
        <button key={value} type="button" aria-label={formatTime(value)} onClick={() => onPick(value)}
          className={kind === 'digital'
            ? 'rounded-xl border-[1.5px] border-ink bg-card text-ink py-3 [@media(min-height:760px)]:py-4 lg:py-6 text-2xl lg:text-3xl font-bold tabular-nums active:scale-95 min-h-[3.5rem]'
            : 'rounded-xl border-[1.5px] border-ink bg-card text-ink p-2 active:scale-95 min-h-[3.5rem] flex justify-center'}
        >
