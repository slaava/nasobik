...
- **`src/db/`** — `idb` wrappers. `schema.ts` defines object stores `profiles`, `cards`, `sessions` with a `by-profile` index. `syncCardsToUnlockedTables` reconciles mul/div cards when the parent toggles tables; cards on still-unlocked tables are preserved so Leitner progress is not lost. It ignores `add`/`sub` cards entirely — those are owned by the session (`putCards` + `deleteCards` in `App.onFinish`).
...
- **Heatmap and `syncCardsToUnlockedTables` must filter to mul/div.** Arith cards share the `a`/`b` fields; an `add` card `3+7` would otherwise collide with the `3×7` cell or get deleted as "not an unlocked table".
